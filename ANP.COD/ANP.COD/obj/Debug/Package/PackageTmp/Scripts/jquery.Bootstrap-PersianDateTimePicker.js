...
(function ($) {
    ...
    function createDateTimePickerHtml($popoverDescriber, dateTimeInJsonFormat, writeDateString) {
       ...
       var entryDate = parsePreviousDateTimeValue($.trim($target.val()));

        //سال شروع بازه
        var k=entryDate.Year-8;
        if(k>currentYear)
        k=currentYear;

        //سال پایان بازه
        var EndYear=entryDate.Year+ 3;
        ...
        for ( k; k <= EndYear ; k++) {
            ...
            if (k == entryDate.Year)
                $dropDownYear.addClass('bg-success');
        ...
        if (dateTimeInJsonFormat.Year <= 0) dateTimeInJsonFormat.Year = 1395;
